...
/**
 * Shown wherever documents are handled. Everything else in the app works
 * without a passphrase — only uploading, opening and reviewing files need one.
 */
export function CryptoGate({ children }: { children: React.ReactNode }) {
  ...
  if (status === 'unavailable') return <CryptoUnavailable />;
  if (status === 'needs-setup') return <CryptoSetup />;
  if (status === 'locked') return <CryptoUnlock />;
  return <>{children}</>;
}
